...
import cn.largequant.cloudcommon.util.AddressUtils;
import cn.largequant.cloudcommon.util.DateUtil;
...
public class WxMobilePayController {
    ...
    public String pay(Product product, ModelMap map) {
        // H5支付(需要公众号内支付
        String url = wxPayService.wxPayMobile(product);
        return "redirect:" + url;
    }
    ...
    public String h5pay(Product product, ModelMap map) {
        ...
        String mweb_url = wxPayService.wxPayH5(product);
        if (StringUtils.isNotBlank(mweb_url)) {
            return "redirect:" + mweb_url;
        } else {
            //自定义错误页面
            return "redirect:https://blog.52itstyle.com";
        }
    }
    ...
    public String dopay(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String notify_url = server_url + "/wxMobile/WXPayBack";//回调接口
        ...
        if ("SUCCESS".equals(returnCode)) {
            ...
            if ("SUCCESS".equals(resultCode)) {
                ...
                url.append("redirect:/wxMobile/payPage?");
                ...
                url.append("redirect:/wxMobile/error?code=0&orderNo=" + orderNo);//该订单已支付
            }
        } else {
            logger.info("订单号:{}错误信息:{}", orderNo, returnMsg);
            url.append("redirect:/wxMobile/error?code=1&orderNo=" + orderNo);//系统错误
        }
        return url.toString();
    }
